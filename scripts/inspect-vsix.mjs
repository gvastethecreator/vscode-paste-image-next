...
const filename = requested
  ...
  : path.join(
      root,
      (await readdir(root)).find((name) => /^paste-image-next(?:-.*)?\.vsix$/.test(name)) || "",
    );
assert.ok(filename.endsWith(".vsix"), "No Paste Image Next VSIX found.");
...
assert.equal(manifest.name, "paste-image-next");
assert.equal(manifest.version, "0.1.0");
assert.equal(manifest.main, "./dist/node/extension.cjs");
assert.equal(manifest.browser, "./dist/web/extension.cjs");
assert.deepEqual(manifest.activationEvents, ["onLanguage"]);
assert.deepEqual(manifest.extensionKind, ["ui", "workspace"]);
assert.equal(manifest.contributes.commands, undefined, "Dead scaffold commands must not ship.");
...
assert.equal(manifest.contributes.configuration.properties["pasteImageNext.markdown.enabled"].default, false);
assert.equal(manifest.contributes.configuration.properties["pasteImageNext.maximumFileSizeMiB"].maximum, 100);
