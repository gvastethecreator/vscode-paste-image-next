...
const vsix = requested
  ...
  : path.join(root, (await readdir(root)).find((name) => /^paste-image-next(?:-.*)?\.vsix$/.test(name)) || "");
if (!vsix.endsWith(".vsix")) throw new Error("Build the Paste Image Next VSIX before the packaged smoke test.");

const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "pin-vsix-"));
